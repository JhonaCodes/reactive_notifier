...
class OrderViewModel extends ViewModel<TestOrderState> {
  ...
  OrderViewModel({this.shouldListenToUser = false}) : super(const TestOrderState(orderId: '', items: []));
...
mixin OrderService {
  ...
  static ReactiveNotifier<OrderViewModel> get orderState {
    _orderState ??= ReactiveNotifier<OrderViewModel>(() => OrderViewModel(shouldListenToUser: true));
    return _orderState!;
  }
  
  static ReactiveNotifier<OrderViewModel> createSimple() {
    return ReactiveNotifier<OrderViewModel>(() => OrderViewModel(shouldListenToUser: false));
  }
...
void main() {
  group('Memory Leak Prevention Tests', () {
    ...
    test('listenVM properly cleans up single listener', () {
      ...
      expect(orderVM.activeListenerCount, equals(0)); // Init hasn't been called yet manually

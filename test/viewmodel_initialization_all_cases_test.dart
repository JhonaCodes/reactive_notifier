...
mixin ComplexAsyncViewModelService {
  ...
  static ReactiveNotifier<ComplexAsyncViewModel> get instance {
    _instance ??= ReactiveNotifier<ComplexAsyncViewModel>(
      () => ComplexAsyncViewModel(),
    );
    return _instance!;
  }
...
Future<void> waitForAsyncInit(
  AsyncViewModelImpl vm, {
  int maxAttempts = 50,
  int delayMs = 20,
}) async {
  int attempts = 0;
  ...
  fail(
    'AsyncViewModel failed to initialize within ${maxAttempts * delayMs}ms. '
...
void main() {
  group('ViewModel<T> & AsyncViewModelImpl<T> Initialization - ALL Cases', () {
    ...
    group('Case 1: Constructor Initialization', () {
      test(
        'ViewModel should initialize WITHOUT context and have default state',
        () {
      ...
      test(
        'AsyncViewModel should initialize complex state correctly',
        () async {
      ...
      test(
        ...
        () async {
          ...
          try {
            await waitForAsyncInit(vm);
            fail(
              'Expected StateError to be thrown during async initialization',
            );
          } catch (e) {
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          try {
              ...
              MaterialApp(
                home:
                    ReactiveViewModelBuilder<
                      ...
                      SimpleState
                    >(
                      viewmodel: ContextDependentService.createNew(
            ...
            expect(
              vm.data.source,
              anyOf(equals('context'), equals('fallback')),
            );
          } catch (e) {
    ...
    group('Case 7: Error Handling During Init', () {
      ...
      test(
        'AsyncViewModel should handle exceptions in init() gracefully',
        () async {
          final vm = BasicAsyncViewModel();
      ...
      testWidgets(
        'AsyncViewModel should handle init errors in widget context',
        (tester) async {
          final vm = BasicAsyncViewModel();
          vm.triggerError();
          ...
          expect(
            find.text('Error: Exception: Test error during async init'),
            findsOneWidget,
          );
        },
      );
    ...
    group('Case 8: Performance and Memory', () {
      test(
        'ViewModel should create and dispose many instances efficiently',
        () {
      ...
      test(
        'AsyncViewModel should handle slow initialization without blocking',
        () async {
      ...
      testWidgets('ViewModel should handle rapid context changes efficiently', (
        ...
        expect(
          vm.initCallCount,
          lessThanOrEqualTo(3),
        ); // Initial + at most 2 reinitializes
        expect(vm.data.source, equals('context'));
      });
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          if (vm.data != null) {
            expect(
              vm.data!.source,
              anyOf(equals('initial'), equals('context')),
            );
          }
        },
      );
    ...
    group('Case 9: Edge Cases and Race Conditions', () {
      ...
      testWidgets(
        'ViewModel should handle widget disposal during reinitialize',
        (tester) async {
          final vm = BasicViewModel();
      ...
      test(
        'ViewModel should maintain state consistency during rapid updates',
        () {
          final vm = BasicViewModel();
    ...
    group('Case 10: State Validation and Consistency', () {
      test(
        'ViewModel should maintain consistent flags throughout lifecycle',
        () {
          // Without context
          final vm = BasicViewModel();
      ...
      test(
        'AsyncViewModel should handle data getter access patterns correctly',
        () async {
          final vm = BasicAsyncViewModel();

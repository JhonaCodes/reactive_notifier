...
void main() {
  group('ReactiveNotifier Instance Management', () {
    ...
    group('Instance Creation and Counting Tests', () {
      test(
          'should create multiple instances of the same type and track them correctly',
          () {
        ...
        expect(ReactiveNotifier.instanceCountByType<int>(), 3,
            reason:
                'Type-specific count should track instances of specific type');

        // Assert: Each instance should maintain its own value
        expect(state1.notifier, 0,
            reason: 'First instance should have its own value');
        expect(state2.notifier, 1,
            reason: 'Second instance should have its own value');
        expect(state3.notifier, 2,
            reason: 'Third instance should have its own value');
      });

      test(
          'should create instances of different types and track them separately',
          () {
        ...
        expect(intState.notifier, 0,
            reason: 'Int instance should have correct value');
        expect(stringState.notifier, 'hello',
            reason: 'String instance should have correct value');
        expect(boolState.notifier, true,
            reason: 'Bool instance should have correct value');
      });
      ...
      test('should handle complex generic types in instance tracking', () {
        ...
        ReactiveNotifier<List<String>>(() => ['a', 'b']);
        ReactiveNotifier<Map<String, int>>(() => {'count': 1});
        ReactiveNotifier<List<Map<String, dynamic>>>(() => [
              {'id': 1}
            ]);
        ...
        expect(
            ReactiveNotifier.instanceCountByType<List<Map<String, dynamic>>>(),
            1,
      ...
      test('should increment instance count with each new creation', () {
        ...
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'Should start with zero instances');
        ...
        expect(ReactiveNotifier.instanceCount, 1,
            reason: 'Count should be 1 after first creation');

        final state2 = ReactiveNotifier<String>(() => 'test');
        expect(ReactiveNotifier.instanceCount, 2,
            reason: 'Count should be 2 after second creation');

        final state3 = ReactiveNotifier<bool>(() => false);
        expect(ReactiveNotifier.instanceCount, 3,
            reason: 'Count should be 3 after third creation');

        // Verify instances are independent
        expect(state1.notifier, 1,
            reason: 'First instance should maintain its value');
        expect(state2.notifier, 'test',
            reason: 'Second instance should maintain its value');
        expect(state3.notifier, false,
            reason: 'Third instance should maintain its value');
      });
    });
    ...
    group('Cleanup and Memory Management Tests', () {
      ...
      test('should allow new instance creation after cleanup', () {
        ...
        expect(ReactiveNotifier.instanceCount, 2,
            reason: 'Initial instances should be tracked');

        ReactiveNotifier.cleanup();
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'Instances should be cleaned up');
        ...
        expect(newIntState.notifier, 42,
            reason: 'New int instance should work correctly');
        expect(newStringState.notifier, 'new',
            reason: 'New string instance should work correctly');
      });
      ...
      test('should handle multiple cleanup calls safely', () {
        ...
        ReactiveNotifier<String>(() => 'test');
        expect(ReactiveNotifier.instanceCount, 2,
            reason: 'Instances should be created');

        // Act: Call cleanup multiple times
        ReactiveNotifier.cleanup();
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'First cleanup should work');

        ReactiveNotifier.cleanup();
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'Second cleanup should be safe');

        ReactiveNotifier.cleanup();
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'Third cleanup should be safe');

        // Assert: Should still allow new creation after multiple cleanups
        final newState = ReactiveNotifier<bool>(() => true);
        expect(ReactiveNotifier.instanceCount, 1,
            reason:
                'New instance creation should work after multiple cleanups');
        expect(newState.notifier, true,
            reason: 'New instance should function correctly');
      });
      ...
      test('should maintain accurate counts during mixed operations', () {
        ...
        ReactiveNotifier<String>(() => 'test');
        expect(ReactiveNotifier.instanceCount, 2,
            reason: 'Two instances should be tracked');

        // Create more instances
        ReactiveNotifier<bool>(() => false);
        expect(ReactiveNotifier.instanceCount, 3,
            reason: 'Three instances should be tracked');

        // Cleanup and verify
        ReactiveNotifier.cleanup();
        expect(ReactiveNotifier.instanceCount, 0,
            reason: 'All instances should be cleaned');

        // Create new instances
        final newState1 = ReactiveNotifier<double>(() => 3.14);
        final newState2 = ReactiveNotifier<List<int>>(() => [1, 2]);
        expect(ReactiveNotifier.instanceCount, 2,
            reason: 'New instances should be tracked');

        // Verify functionality is preserved
        expect(newState1.notifier, 3.14,
            reason: 'New double instance should work');
        expect(newState2.notifier, [1, 2],
            reason: 'New list instance should work');
      });
    });

  ...
  group('ReactiveNotifier Basic Functionality', () {
    ...
    group('Initialization Tests', () {
      test('should initialize with default value from factory function', () {
        ...
        expect(state.notifier, 0, 
            reason: 'ReactiveNotifier should initialize with factory function result');
      ...
      test('should support different data types during initialization', () {
        ...
        expect(intState.notifier, 42, reason: 'Int notifier should initialize correctly');
        expect(stringState.notifier, 'hello', reason: 'String notifier should initialize correctly');
        expect(boolState.notifier, true, reason: 'Bool notifier should initialize correctly');
        expect(listState.notifier, [1, 2, 3], reason: 'List notifier should initialize correctly');
    ...
    group('Basic State Update Tests', () {
      ...
      test('should handle multiple consecutive state updates', () {
        ...
        expect(state.notifier, 'second', reason: 'Should update to second value');
      ...
      test('should handle updateState() with same value', () {
        ...
        expect(state.notifier, 5, 
            reason: 'State should remain consistent when updated with same value');
      });

      test('should update complex objects correctly', () {
        // Setup: Create ReactiveNotifier with complex object
        final state = ReactiveNotifier<Map<String, dynamic>>(() => {'count': 0, 'name': 'test'});
        ...
        expect(state.notifier['name'], 'updated', reason: 'Name should be updated');
        expect(state.notifier['active'], true, reason: 'New property should be added');
        expect(state.notifier.length, 3, reason: 'Map should have all properties');
      });
    ...
    group('State Value Access Tests', () {
      test('should provide consistent access to current value via notifier getter', () {
        // Setup: Create ReactiveNotifier
        final state = ReactiveNotifier<double>(() => 3.14);
        
        // Act & Assert: Multiple accesses should return same value
        expect(state.notifier, 3.14, reason: 'First access should return initial value');
        expect(state.notifier, 3.14, reason: 'Second access should return same value');
        expect(state.notifier, 3.14, reason: 'Third access should return same value');
        
        // Act: Update and verify consistent access
        state.updateState(2.71);
        expect(state.notifier, 2.71, reason: 'Access after update should return new value');
        expect(state.notifier, 2.71, reason: 'Subsequent access should remain consistent');
      });

      test('should maintain value integrity across multiple reads', () {
        // Setup: Create ReactiveNotifier with complex data
        final testData = {'users': ['Alice', 'Bob'], 'count': 2};
        ...
        expect(read1, equals(read2), reason: 'Multiple reads should return equal data');
        expect(read2, equals(read3), reason: 'Subsequent reads should remain equal');
        expect(read1['users'], ['Alice', 'Bob'], reason: 'Nested data should be preserved');
        expect(read1['count'], 2, reason: 'All properties should be maintained');
      });
    });
  });
}
  ...
  group('ReactiveNotifier Computed States', () {
    ...
    group('Basic Computed States', () {
      test('should handle simple computed states correctly', () {
        ...
        final computedState = ReactiveNotifier<int>(
          () => baseState.notifier * 2,
        );
      ...
      test('should handle computed states with conditional logic', () {
        ...
        expect(
          passFailState.notifier,
          true,
          reason: 'Should pass with score 95',
        );
        expect(
          messageState.notifier,
          'Score: 95, Grade: A, Status: PASSED',
        ...
        expect(
          messageState.notifier,
          'Score: 45, Grade: F, Status: FAILED',
        ...
        expect(
          messageState.notifier,
          'Score: 60, Grade: D, Status: PASSED',
    ...
    group('Multi-Level Computed State Dependencies', () {
      ...
      test('should handle complex dependency networks correctly', () {
        ...
        expect(
          finalResult.notifier,
          'A:4, B:3, Sum:7, Product:12, Power:49',
        ...
        expect(
          finalResult.notifier,
          'A:4, B:5, Sum:9, Product:20, Power:81',
      ...
      test('should handle computed states with shared dependencies', () {
        ...
        expect(
          updateHistory.contains('base_updated'),
          true,
          reason: 'Base update should be recorded',
        );
        expect(
          updateHistory.where((h) => h == 'combined_updated').length,
          3,
          reason:
    ...
    group('Dependency Injection and Service Patterns', () {
      test('should support dependency injection patterns', () {
        ...
        serviceState.addListener(() {
          dependentState.updateState(
            '${serviceState.notifier} and $injectedDependency',
          );
        });

        // Assert: Initial state should use defaults
        expect(
          serviceState.notifier,
          'Initial',
          reason: 'Service should start with initial state',
        );
        expect(
          dependentState.notifier,
          'Initial',
        ...
        expect(
          configState.notifier,
          'production_config',
          reason: 'Configuration should be updated',
        );
        expect(
          serviceState.notifier,
          'Service with production_config',
    ...
    group('Performance Optimization for Computed Networks', () {
      test('should efficiently handle large computed state networks', () {
        ...
        final sourceStates = List.generate(
          10,
          (i) => ReactiveNotifier<int>(() => i),
        );
        final computedStates = List.generate(
          10,
          (i) => ReactiveNotifier<int>(() => 0),
        );
        ...
        expect(
          totalUpdates,
          greaterThan(sourceStates.length),
          reason: 'Should have updates for sources and aggregations',
        );

        // Verify final aggregation is correct
        final expectedSum = computedStates.fold<int>(
          0,
          (sum, state) => sum + state.notifier,
        );
        expect(
          finalAggregation.notifier,
          expectedSum,
      ...
      test('should optimize redundant computations in computed networks', () {
        ...
        expect(
          finalResult.notifier,
          'Base:8, Expensive:64, Cheap1:16, Cheap2:24',

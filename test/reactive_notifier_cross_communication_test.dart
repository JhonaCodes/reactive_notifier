  ...
  group('ReactiveNotifier Cross-Communication', () {
    ...
    group('Basic Cross-Notifier Dependencies', () {
      ...
      test('should handle multiple dependent notifiers from single source', () {
        ...
        final descriptionNotifier = ReactiveNotifier<String>(
          () => 'positive even',
        );
      ...
      test('should handle one-to-many and many-to-one relationships', () {
        ...
        final resultNotifier = ReactiveNotifier<String>(
          () => 'sum: 8, product: 15',
        );
    ...
    group('Cascading Updates and Reactive Chains', () {
      test('should handle cascading updates through multiple levels', () {
        ...
        temperatureCelsius.addListener(() {
          temperatureFahrenheit.updateState(
            temperatureCelsius.notifier * 9 / 5 + 32,
          );
        });
      ...
      test('should handle complex reactive chains with branching', () {
        ...
        final validationMessageNotifier = ReactiveNotifier<String>(
          () => 'Invalid input',
        );
        final processedResultNotifier = ReactiveNotifier<String>(
          () => 'No result',
        );
        ...
        isValidNotifier.addListener(() {
          if (isValidNotifier.notifier) {
            ...
            validationMessageNotifier.updateState(
        ...
        expect(
          parsedNumberNotifier.notifier,
          15,
          reason: 'Should parse positive number',
        );
        expect(
          isValidNotifier.notifier,
          true,
          reason: 'Should recognize valid input',
        );
        expect(
          validationMessageNotifier.notifier,
          'Valid number',
          reason: 'Should show valid message',
        );
        expect(
          processedResultNotifier.notifier,
          'Positive: 30',
    ...
    group('Circular Dependencies and Loop Prevention', () {
      ...
      test('should prevent stack overflow in deeply nested updates', () {
        ...
        final notifiers = List.generate(
          10,
          (index) => ReactiveNotifier<int>(() => index),
        );
        ...
        for (int i = 0; i < notifiers.length - 1; i++) {
          ...
          notifiers[i].addListener(() {
            ...
            if (totalUpdates < 20) {
              notifiers[currentIndex + 1].updateState(
                notifiers[currentIndex].notifier + 1,
              );
            }
          });
        }
    ...
    group('Performance and Coordination Tests', () {
      test(
        ...
        () {
          ...
          for (int i = 0; i < systems.length; i++) {
            expect(
              (systems[i]['source'] as ReactiveNotifier<int>).notifier,
              i * 10,
              reason: 'Source $i should have correct value',
            );
            expect(
              (systems[i]['derived'] as ReactiveNotifier<String>).notifier,
              'derived_${i * 10}',

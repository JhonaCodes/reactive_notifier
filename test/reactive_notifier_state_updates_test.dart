  ...
  group('ReactiveNotifier State Updates', () {
    ...
    group('updateSilently() Method Tests', () {
      ...
      test('should update Map value without notifying listeners', () {
        ...
        final notifier = ReactiveNotifier<Map<String, dynamic>>(() => {'key': 'value'});
        var listenerCallCount = 0;
      ...
      test('should update complex custom object without notifying listeners', () {
        ...
        final notifier = ReactiveNotifier<TestModel>(() => TestModel(id: 1, name: 'test'));
        var listenerCallCount = 0;
      ...
      test('should allow multiple silent updates without accumulating notifications', () {
      ...
      test('should work correctly when mixed with regular updates', () {
        ...
        expect(receivedValues, equals([20, 40]),
            reason: 'Listeners should only receive values from regular updates');
      });
    });
    ...
    group('transformState() Method Tests', () {
      ...
      test('should transform List value using function and notify listeners', () {
        ...
        notifier.addListener(() {
          listenerCallCount++;
          receivedValues.add(List.from(notifier.notifier)); // Copy to avoid reference issues
        });
        ...
        expect(listenerCallCount, equals(1));
        expect(receivedValues, equals([[1, 2, 3, 4]]));
      });

      test('should transform Map value using function and notify listeners', () {
        // Setup: Create ReactiveNotifier with Map value
        final notifier = ReactiveNotifier<Map<String, int>>(() => {'a': 1, 'b': 2});
        var listenerCallCount = 0;
      ...
      test('should handle complex transformations with custom objects', () {
        // Setup: Create ReactiveNotifier with custom object
        final notifier = ReactiveNotifier<TestModel>(() => TestModel(id: 1, name: 'original'));
        var listenerCallCount = 0;
    ...
    group('transformStateSilently() Method Tests', () {
      test('should transform int value using function without notifying listeners', () {
        // Setup: Create ReactiveNotifier with int value and add listener
        final notifier = ReactiveNotifier<int>(() => 5);
        var listenerCallCount = 0;
        final receivedValues = <int>[];
        ...
        notifier.transformStateSilently((currentValue) => currentValue * currentValue);
        
        // Assert: Value should be transformed but listeners should NOT be called
        expect(notifier.notifier, equals(25), 
            reason: 'Value should be transformed by the function');
        expect(listenerCallCount, equals(0), 
            reason: 'Listeners should NOT be called when transforming silently');
        expect(receivedValues, isEmpty,
            reason: 'No values should be received by listeners');
      });

      test('should transform String value using function without notifying listeners', () {
        // Setup: Create ReactiveNotifier with String value
        ...
        notifier.transformStateSilently((currentValue) => 'hello $currentValue');
      ...
      test('should transform List value using function without notifying listeners', () {
        // Setup: Create ReactiveNotifier with List value
        ...
        notifier.transformStateSilently((currentList) => 
            currentList.map((item) => item * 2).toList());
      ...
      test('should transform Map value using function without notifying listeners', () {
        // Setup: Create ReactiveNotifier with Map value
        final notifier = ReactiveNotifier<Map<String, int>>(() => {'x': 1, 'y': 2});
        var listenerCallCount = 0;
      ...
      test('should handle complex transformations with custom objects without notification', () {
        // Setup: Create ReactiveNotifier with custom object
        final notifier = ReactiveNotifier<TestModel>(() => TestModel(id: 100, name: 'test'));
        var listenerCallCount = 0;
        ...
        notifier.transformStateSilently((currentModel) => TestModel(
          id: currentModel.id * 2, 
          name: '${currentModel.name}_silent'
        ));
      ...
      test('should allow multiple silent transformations without accumulating notifications', () {
        // Setup: Create ReactiveNotifier with int value
      ...
      test('should work correctly when mixed with regular transformations', () {
        ...
        notifier.transformStateSilently((value) => value * 2);     // 10 -> 20 (silent)
        notifier.transformState((value) => value + 5);             // 20 -> 25 (notify)
        notifier.transformStateSilently((value) => value * 2);     // 25 -> 50 (silent)
        ...
        expect(receivedValues, equals([25, 40]),
            reason: 'Listeners should only receive values from regular transformations');
      });

      test('should maintain function purity - original state not modified in silent transformation', () {
        // Setup: Create ReactiveNotifier with mutable object
        ...
        notifier.transformStateSilently((currentList) {
          return currentList.map((item) => item ~/ 2).toList(); // Use integer division
        });
        
        // Assert: Original reference should not be modified
        expect(originalList, equals([10, 20, 30]), 
            reason: 'Original list should not be modified by silent transformation');

...
void main() {
  group('AsyncViewModelImpl<T> Tests', () {
    ...
    group('AsyncState Management Tests', () {
      test('should initialize with AsyncState.initial() when loadOnInit is false', () async {
        // Setup: Create AsyncViewModel with loadOnInit disabled
        final viewModel = TestAsyncViewModel(initialData: 'test_data', loadOnInit: false);
        ...
        expect(viewModel.hasInitializedListenerExecution, isFalse,
            reason: 'Listener execution should not be initialized when loadOnInit is false');
      });
      ...
      test('should handle AsyncState.success correctly', () async {
        ...
        final viewModel = TestAsyncViewModel(initialData: 'success_data', loadOnInit: false);
      ...
      test('should handle AsyncState.loading correctly', () async {
        // Setup: Create AsyncViewModel without auto-load
        final viewModel = TestAsyncViewModel(initialData: 'test', loadOnInit: false);
    ...
    group('AsyncViewModel Lifecycle Tests', () {
      test('should call init() and setup listeners during reload()', () async {
        // Setup: Create AsyncViewModel without auto-load
        final viewModel = TestAsyncViewModel(initialData: 'reload_test', loadOnInit: false);
        expect(viewModel.initCallCount, equals(0), reason: 'init() should not be called initially');
        expect(viewModel.setupListenersCallCount, equals(0), reason: 'setupListeners() should not be called initially');
      ...
      test('should handle onResume callback after successful initialization', () async {
      ...
      test('should prevent concurrent reloads when already loading', () async {
        ...
        expect(viewModel.initCallCount, equals(1), 
            reason: 'Concurrent reload should be prevented - init() called only once');
      });
      ...
      test('should handle dispose correctly', () async {
        ...
        await Future.delayed(const Duration(milliseconds: 10)); // Wait for initialization
    ...
    group('AsyncViewModel State Update Tests', () {
      test('should update state and notify listeners with updateState()', () async {
        // Setup: Create AsyncViewModel and add listener
        final viewModel = TestAsyncViewModel(initialData: 'initial', loadOnInit: false);
      ...
      test('should update state without notifying listeners with updateSilently()', () async {
        // Setup: Create AsyncViewModel and add listener
        final viewModel = TestAsyncViewModel(initialData: 'initial', loadOnInit: false);
        var listenerCallCount = 0;
    ...
    group('AsyncViewModel State Transformation Tests', () {
      test('should transform to success state with transformState() when result has data', () async {
        // Setup: Create AsyncViewModel with success state
        final viewModel = TestAsyncViewModel(initialData: 'transform_test', loadOnInit: false);
        ...
        viewModel.transformState((currentState) => AsyncState.success('transformed_data'));
      ...
      test('should transform data with transformDataState()', () async {
        ...
        final viewModel = ListAsyncViewModel(initialData: [1, 2, 3], loadOnInit: false);
      ...
      test('should transform data silently with transformDataStateSilently()', () async {
        // Setup: Create AsyncViewModel with List data
        final viewModel = ListAsyncViewModel(initialData: [10, 20], loadOnInit: false);
        ...
        expect(viewModel.isSuccess(), isTrue);
        expect(listenerCallCount, equals(0), 
            reason: 'Listeners should NOT be notified with transformDataStateSilently');
      });

      test('should transform entire state silently with transformStateSilently()', () async {
        // Setup: Create AsyncViewModel with success state
        final viewModel = TestAsyncViewModel(initialData: 'test', loadOnInit: false);
        viewModel.updateState('success_data');
        var listenerCallCount = 0;
        ...
        viewModel.transformStateSilently((currentState) => AsyncState.loading());
      ...
      test('should transform to error state silently with transformStateSilently()', () async {
        // Setup: Create AsyncViewModel with success state
        final viewModel = TestAsyncViewModel(initialData: 'test', loadOnInit: false);
        viewModel.updateState('success_data');
        var listenerCallCount = 0;
        ...
        viewModel.transformStateSilently((currentState) => AsyncState.error('Silent error'));
      ...
      test('should handle null return in transformDataState gracefully', () async {
    ...
    group('AsyncViewModel Cross-Communication Tests', () {
      test('should setup reactive communication with listenVM()', () async {
        ...
        final sourceViewModel = TestAsyncViewModel(initialData: 'source_data', loadOnInit: false);
        ...
        final currentState = await dependentViewModel.listenToSource(sourceViewModel);
        ...
        expect(dependentViewModel.receivedSourceState?.isSuccess, isTrue, 
            reason: 'Dependent AsyncViewModel should receive current source state');
        ...
        expect(dependentViewModel.receivedSourceState?.data, equals('source_updated'));
        expect(dependentViewModel.sourceUpdateCount, equals(2), 
            reason: 'Should receive initial + update = 2 calls');
      });

      test('should stop reactive communication with stopListeningVM()', () async {
        // Setup: Create source and dependent AsyncViewModels
        final sourceViewModel = TestAsyncViewModel(initialData: 'source', loadOnInit: false);
        final dependentViewModel = DependentAsyncViewModel(loadOnInit: false);
    ...
    group('AsyncViewModel Error Handling Tests', () {
      ...
      test('should handle reload errors gracefully', () async {
        ...
        final viewModel = ConditionalErrorAsyncViewModel(shouldFail: false, loadOnInit: false);

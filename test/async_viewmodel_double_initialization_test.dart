...
class DoubleInitializationTestViewModel extends AsyncViewModelImpl<String> {
  DoubleInitializationTestViewModel() : super(AsyncState.initial(), loadOnInit: true);
...
void main() {
  group('AsyncViewModelImpl Double Initialization Prevention Tests', () {
    ...
    test('should not double-initialize when reinitializeWithContext is called multiple times', () async {
      ...
      expect(viewModel.initCallCount, equals(1), reason: 'Should initialize only once initially');
      expect(viewModel.hasData, isTrue, reason: 'Should have data after initialization');
      expect(viewModel.data, equals('Initialized 1 times'), reason: 'Should contain correct initial data');
      ...
      expect(viewModel.initCallCount, equals(1), reason: 'Should NOT initialize again when context already available');
      expect(viewModel.data, equals('Initialized 1 times'), reason: 'Data should remain the same');
    });

    test('should maintain initialization state correctly through multiple reinitializeWithContext calls', () async {
      final viewModel = DoubleInitTestService.instance.notifier;
      ...
      // Check initial state
      expect(viewModel.initCallCount, equals(1), reason: 'Should initialize once initially');
      expect(viewModel.hasInitializedListenerExecution, isTrue, reason: 'hasInitializedListenerExecution should be true');
      ...
      expect(viewModel.initCallCount, equals(1), reason: 'Should not reinitialize when already initialized with context');
      expect(viewModel.data, equals('Initialized 1 times'), reason: 'Data should remain unchanged');
    });
  });
}